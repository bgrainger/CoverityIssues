...
Task Build {
  Exec { msbuild /m:4 /property:Configuration=Release CoverityIssues.sln }
}
...
Task Coverity -depends Build {
  ...
  & "C:\Program Files\Coverity\Coverity Static Analysis\bin\cov-commit-defects.exe" `--host coverity `--dataport 9090 `--stream CoverityIssues `--user logosutilityreporter `--password password `--dir build\coverity `--version $headSha `--strip-path $(get-location).Path
  if ($LastExitCode -ne 0) {
